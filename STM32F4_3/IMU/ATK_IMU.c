								...
								float dt)
{
	...
	{
		//如果旋转速率大于限制值则停止积分
		if (spin_rate_sq < sq(SPIN_RATE_LIMIT*angle_to_rad)) 
		{
...
void imuUpdateAttitude(_angle* att,
						_F32xyz* acc,
						_F32xyz* gyro,
						_F32xyz* mag,float dt)
{
	
	//角速度单位由度转为弧度
	gyro->x = gyro->x * angle_to_rad;
	gyro->y = gyro->y * angle_to_rad;
	gyro->z = gyro->z * angle_to_rad;
