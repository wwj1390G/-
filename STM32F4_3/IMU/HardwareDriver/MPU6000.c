...
#define DISABLE_MPU6000()	GPIO_SetBits(GPIOC, GPIO_Pin_5);
#define ENABLE_MPU6000()   	GPIO_ResetBits(GPIOC, GPIO_Pin_5);
...
bool mpu6000GyroRead(_S16xyz* gyroRaw)
{
	if(!isInit) 
		return false;
	u8 buffer[6];
...
bool mpu6000AccRead(_S16xyz* accRaw)
{
	if(!isInit) 
		return false;
	u8 buffer[6];
